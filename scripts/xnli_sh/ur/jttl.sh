...
OUT=models/xnli/jttls/ur/mBart_ur_en_50m2o_batch1_lr3-6_seed_1
...
mkdir -p $OUT
python -m scripts.training_sample --task XNLI --train_data $TRAIN --validation_data $DEV --test_data $TEST --src ur --tgt en --save_dir $OUT --batch_size 1 --freeze_strategy fix_nmt_dec_lm_enc --gpus 1 --tokenizer $PRETRAINED_LM_EN --max_input_len 85 --max_output_len 85 --warmup 0 --lr 0.000003 --model_lm_path $PRETRAINED_LM_EN --model_seq2seq_path $PRETRAINED_MT --epochs 10 --seed 1234 > $OUT/log_results.txt

for SEED in 2 3; do

  OUT=models/xnli/jttls/ur/mBart_ur_en_50m2o_batch1_lr3-6_seed_$SEED
