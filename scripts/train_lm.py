...
from transformers import AutoTokenizer, AutoModelForSeq2SeqLM, AutoConfig, AutoModelForSequenceClassification, \
    MBartTokenizer, MBart50Tokenizer, XLMRobertaTokenizer
...
class LmForTextClassification(pl.LightningModule):
    ...
    def __init__(self, params):
        ...
        if isinstance(self.tokenizer, MBart50Tokenizer):
            print('mbart')
            self.lm_type = 'mbart'
        elif isinstance(self.tokenizer, XLMRobertaTokenizer):
            print('XLM-R')
            self.lm_type = 'xlm-r'
        else:
            print('other')
    ...
    def forward(self, input_ids, attention_mask, token_type_ids, output_label):
        labels = output_label.clone()
        # print(input_ids.size())

        if self.lm_type == 'mbart':
            outputs = self.model(
                input_ids,
        ...
        if self.args.label_smoothing == 0:
            assert lm_logits.shape[-1] == self.num_out_classes  # It has to be the same as the output class
    ...
    def validation_epoch_end(self, outputs, test=None):
        for p in self.model.parameters():
        ...
        if test is not True:
            if self.args.task != 'MultiEurlex':
                self.save_language_model(logs['vaccuracy'])
            else:
                self.save_language_model(logs['vmrp'])
        ...
        if self.args.task != 'MultiEurlex':
            ...
            return {'avg_val_loss': logs['vloss'], 'avg_accuracy': logs['vaccuracy'], 'avg_mrp': logs['vmrp'],
                    'avg_recall': logs['vrecall'], 'avg_f1_score': logs['vf1_score'], 'log': logs, 'progress_bar': logs}
    ...
    def _get_dataloader(self, current_dataloader, split_name, is_train):
        ...
        if self.args.task == 'XNLI':
            dataset = XNLIDataset(hf_dataset=self.hf_datasets[split_name], tokenizer=self.tokenizer,
                                  max_input_len=self.args.max_input_len, label_dict=self.label_dict)
            selec_collate_fn = XNLIDataset.collate_fn
    ...
    def add_model_specific_args(parser, root_dir):
        ...
        parser.add_argument("--model_lm_path", type=str, default='../pretrained_lm/facebook-bart-base',
                            help="Path to the checkpoint directory or model name")
        parser.add_argument("--tokenizer", type=str, default='../pretrained_lm/facebook-bart-base')
...
def main(args):
    ...
    if args.from_pretrained is not None:
        model = LmForTextClassification.load_from_checkpoint(args.from_pretrained)  # , args)
    else:
        model = LmForTextClassification(args)
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        filename='check-{epoch:02d}-{avg_val_accuracy:.2f}',
        save_top_k=1,
        verbose=True,
...
if __name__ == "__main__":
    main_arg_parser = argparse.ArgumentParser(description="language_model")
    parser = LmForTextClassification.add_model_specific_args(main_arg_parser, os.getcwd())
