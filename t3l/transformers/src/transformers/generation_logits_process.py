...
class MinLengthLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        if cur_len < self.min_length:
            scores[:, self.eos_token_id] = torch.tensor(-float("inf"), device=scores.device)
        return scores
...
class NoRepeatNGramLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        for i, banned_tokens in enumerate(banned_batch_tokens):
            scores[i, banned_tokens] = torch.tensor(-float("inf"), device=scores.device)
...
class ForcedBOSTokenLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        if cur_len == 1:
            ...
            scores[:, [i for i in range(num_tokens) if i != self.bos_token_id]] = torch.tensor(-float("inf"),
                                                                                               device=scores.device)
            scores[:, self.bos_token_id] = torch.tensor(0, device=scores.device)
        return scores
...
class ForcedEOSTokenLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        if cur_len == self.max_length - 1:
            num_tokens = scores.shape[1]
            scores[:, [i for i in range(num_tokens) if i != self.eos_token_id]] = torch.tensor(-float("inf"),
                                                                                               device=scores.device)
            scores[:, self.eos_token_id] = torch.tensor(0, device=scores.device)
        return scores


class MyForcedEOSTokenLogitsProcessor(LogitsProcessor):
    r"""
    :class:`~transformers.LogitsProcessor` that enforces the specified token as the last generated token when
    :obj:`max_length` is reached.
